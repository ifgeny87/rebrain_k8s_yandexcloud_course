...
func getRequests(w http.ResponseWriter, r *http.Request) {
	stmt := `SELECT id, name, description, created_at, updated_at, video_url, text_url FROM requests WHERE not archived`
	...
	for rows.Next() {
		request := Request{}
		err = rows.Scan(&request.ID, &request.Name, &request.Description, &request.CreatedAt, &request.UpdatedAt, &request.VideoURL, &request.TextURL)
		if err != nil {
